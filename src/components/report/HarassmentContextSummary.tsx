...
export const HarassmentContextSummary: React.FC<HarassmentContextSummaryProps> = ({
  ...
  return (
    ...
    >
      <h2 id="harassment-context-heading" className="text-[var(--type-fixed-16)] font-bold text-ui-content-primary">
        ...
        <dl className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {rows.map((row) => (
            <div key={row.label} className="rounded-[var(--radius-badge-md)] bg-ui-surface-subtle border border-ui-stroke-subtle p-3">
              <dt className="text-[var(--type-fixed-12)] font-semibold text-ui-content-muted">{row.label}</dt>
              <dd className="mt-1 text-[var(--type-fixed-14)] font-semibold text-ui-content-primary">{row.value}</dd>
            </div>
          ))}
        </dl>
      )}
      {report.reporterName && (
        <div className="pt-3 border-t border-ui-stroke-subtle space-y-1">
          <p className="text-[var(--type-fixed-12)] font-semibold text-ui-content-muted">
            {language === 'bn' ? 'প্রকাশ্য প্রতিবেদকের নাম' : 'Public reporter name'}
          </p>
          <p className="text-[var(--type-fixed-14)] font-semibold text-ui-content-primary">{report.reporterName}</p>
