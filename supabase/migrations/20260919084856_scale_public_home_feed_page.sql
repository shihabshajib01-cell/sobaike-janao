-- Scalable public Home feed architecture.
-- Production-verified on 2026-09-19 before source alignment:
-- * exact JSONB parity across all/latest/popular/most_shared
-- * EN/BN district parity
-- * location-aware ordering parity
-- * visible-page JSON construction only
-- * PostGIS KNN location ranking
--
-- Keep public.get_public_home_feed() unchanged as the compatibility fallback.

create extension if not exists postgis with schema extensions;

CREATE INDEX IF NOT EXISTS idx_complaints_public_feed_district_latest ON public.complaints USING btree (lower(district), created_at DESC, id DESC) WHERE ((status = 'published'::text) AND ((publication_preferences -> 'showGeneralLocation'::text) = 'true'::jsonb));

CREATE INDEX IF NOT EXISTS idx_complaints_public_feed_district_popular ON public.complaints USING btree (lower(district), public_view_count DESC, public_share_count DESC, created_at DESC, id DESC) WHERE ((status = 'published'::text) AND ((publication_preferences -> 'showGeneralLocation'::text) = 'true'::jsonb));

CREATE INDEX IF NOT EXISTS idx_complaints_public_feed_district_shared ON public.complaints USING btree (lower(district), public_share_count DESC, public_view_count DESC, created_at DESC, id DESC) WHERE ((status = 'published'::text) AND ((publication_preferences -> 'showGeneralLocation'::text) = 'true'::jsonb));

CREATE INDEX IF NOT EXISTS idx_complaints_public_feed_geo ON public.complaints USING gist (((st_setsrid(st_makepoint(longitude, latitude), 4326))::geography)) WHERE ((status = 'published'::text) AND ((publication_preferences -> 'showGeneralLocation'::text) = 'true'::jsonb) AND (latitude IS NOT NULL) AND (longitude IS NOT NULL) AND (NOT ((latitude = (0.0)::double precision) AND (longitude = (0.0)::double precision))) AND ((latitude >= ('-90.0'::numeric)::double precision) AND (latitude <= (90.0)::double precision)) AND ((longitude >= ('-180.0'::numeric)::double precision) AND (longitude <= (180.0)::double precision)));

CREATE INDEX IF NOT EXISTS idx_complaints_public_feed_popular ON public.complaints USING btree (public_view_count DESC, public_share_count DESC, created_at DESC, id DESC) WHERE (status = 'published'::text);

CREATE INDEX IF NOT EXISTS idx_complaints_public_feed_shared ON public.complaints USING btree (public_share_count DESC, public_view_count DESC, created_at DESC, id DESC) WHERE (status = 'published'::text);
...
begin
  ...
  if v_filter='all' and v_has_visitor_loc then
    ...
    where c.status='published'
      and (
        v_clean_district='all'
        or lower(coalesce(c.district,''))=v_clean_district
      )
    ...
    if v_offset < v_geo_count then
      ...
      if cardinality(v_candidate_ids) < v_limit + 1 then
        ...
        v_candidate_ids :=
          v_candidate_ids || coalesce(v_fallback_ids,'{}'::text[]);
      end if;
    else
      select coalesce(
        array_agg(q.id order by q.created_at desc,q.id desc),
    ...
    from (
      select
        c.id,
        c.created_at,
        coalesce(c.public_view_count,0) as views,
        coalesce(c.public_share_count,0) as shares,
      ...
      order by
        coalesce(c.public_view_count,0) desc,
        coalesce(c.public_share_count,0) desc,
    ...
    select coalesce(
      array_agg(
        q.id
        order by
          q.shares desc nulls last,
          q.views desc nulls last,
          q.created_at desc,
          q.id desc
      ),
      '{}'::text[]
    ...
    from (
      ...
      order by
        c.public_share_count desc nulls last,
        c.public_view_count desc nulls last,
        c.created_at desc,
        c.id desc
      offset v_offset
      limit v_limit + 1
...
revoke all privileges
  on function public.get_public_home_feed_page(
...
comment on function public.get_public_home_feed_page(
  double precision,double precision,text,text,integer,integer
) is
  'Scalable public Home feed page: ranks indexed complaint IDs first, then builds public JSON only for the requested page; uses PostGIS KNN for location-first feed ordering.';
