...
The map publishes **543 canonical polygon matches** and explicitly marks **58 canonical records without a verified polygon**. Every one of the 601 records remains selectable in its correct district. Missing boundaries are never guessed, split, merged, or borrowed from a parent area.

Of the 543 published polygons:
...
- **1** previously verified polygon (Lakshmipur → Raipur) is retained from the user-provided `bd_district_map(1).html`.
