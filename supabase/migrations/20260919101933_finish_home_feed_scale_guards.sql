-- Align Home feed ordering/filter expressions with the indexes that protect
-- thousands-of-report scaling. Preserve the public response/ranking contract.

drop index if exists public.idx_complaints_public_feed_popular;
create index idx_complaints_public_feed_popular
  on public.complaints (
    public_view_count desc,
    public_share_count desc,
    (date_trunc('second', created_at at time zone 'UTC')) desc,
    created_at desc,
    id desc
  )
  where status='published';

drop index if exists public.idx_complaints_public_feed_district_popular;
create index idx_complaints_public_feed_district_popular
  on public.complaints (
    lower(district),
    public_view_count desc,
    public_share_count desc,
