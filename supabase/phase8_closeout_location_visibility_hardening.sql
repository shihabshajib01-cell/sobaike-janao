-- =============================================================================
-- SOBAIKE JANAO — PHASE 8 CLOSEOUT LOCATION VISIBILITY HARDENING
--
-- Purpose:
--   1. Keep visitor GPS transient and server-side ranking-only.
--   2. Keep raw complaint coordinates and derived distance out of public payloads.
--   3. Respect publication_preferences.showGeneralLocation for all public
--      district / area / formatted-location text.
--   4. Prevent an explicitly selected Home district from indirectly revealing
--      a report whose general location was not approved for publication.
--
-- This is a forward hardening migration. It preserves the existing Phase 8
-- ranking behavior, filters, public payload shape, permissions, and all
-- unrelated complaint / response workflows.
-- =============================================================================

-- -----------------------------------------------------------------------------
-- 1. Shadow-ranked Home feed
-- -----------------------------------------------------------------------------
...
BEGIN
  ...
  WITH base_complaints AS (
    SELECT
      ...
      CASE
        WHEN (c.publication_preferences->'showGeneralLocation') = 'true'::jsonb
        THEN coalesce(c.formatted_address, c.area, c.district)
        ELSE NULL
...
-- -----------------------------------------------------------------------------
-- 2. Coordinate-free, publication-preference-safe public list
-- -----------------------------------------------------------------------------
  ...
  SELECT coalesce(
    jsonb_agg(
      jsonb_build_object(
        ...
        'district', CASE
          WHEN (c.publication_preferences->'showGeneralLocation') = 'true'::jsonb
          THEN c.district ELSE NULL END,
        'area', CASE
          WHEN (c.publication_preferences->'showGeneralLocation') = 'true'::jsonb
          THEN c.area ELSE NULL END,
        'location', CASE
          WHEN (c.publication_preferences->'showGeneralLocation') = 'true'::jsonb
          THEN coalesce(c.formatted_address, c.area, c.district) ELSE NULL END,
...
-- -----------------------------------------------------------------------------
-- 3. Coordinate-free, publication-preference-safe public detail
-- -----------------------------------------------------------------------------
...
-- Preserve the existing public RPC access model. Direct complaint-table access
-- remains governed separately by table grants + RLS.
