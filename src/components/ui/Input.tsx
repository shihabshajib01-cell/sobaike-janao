  ...
  ({ id, label, helperText, error, leftIcon, rightIcon, required, disabled, className = '', ...props }, ref) => {
    ...
    return (
      <div className="w-full text-left">
        {label && (
          <label htmlFor={inputId} className="block type-label text-primary mb-1.5">
            {label}
        ...
        <div className="relative flex items-center">
          ...
          <input
            ...
            className={`w-full min-h-[44px] bg-surface text-primary placeholder:text-muted border ui-radius-control ui-space-field-y transition-colors focus:outline-none focus:ring-2 focus:ring-[var(--ui-focus)] focus:border-[var(--ui-accent)] disabled:bg-surface-subtle disabled:text-muted disabled:cursor-not-allowed ${
        ...
        {!error && helperText && (
          <p id={`${inputId}-helper`} className="mt-1.5 type-helper text-muted">
            {helperText}
          </p>
        )}
      </div>
