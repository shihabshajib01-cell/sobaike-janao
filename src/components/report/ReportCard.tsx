...
export const ReportCard: React.FC<ReportCardProps> = ({ report, className = '' }) => {
  ...
  return (
    ...
    >
      ...
      <h3 className="text-[var(--type-fixed-16)] sm:text-[var(--type-fixed-17)] md:text-[var(--type-fixed-20)] leading-[var(--type-line-ratio-138)] sm:leading-[var(--type-line-ratio-140)] md:leading-[var(--type-line-30)] font-[var(--font-weight-bold)] md:font-[var(--font-weight-semibold)] text-ui-content-primary transition-colors line-clamp-2 break-words">
        {title}
      </h3>
      ...
      {report.reportedSubject && (
        <div className="text-[var(--type-fixed-12)] sm:text-[var(--type-fixed-13)] md:text-[var(--type-fixed-14)] text-ui-content-secondary flex items-center gap-1.5 flex-wrap min-w-0">
          <p className="text-ui-content-muted font-[var(--font-weight-regular)] text-[var(--type-fixed-115)] sm:text-[var(--type-fixed-12)] md:text-[var(--type-fixed-13)]">
            {language === 'bn' ? 'প্রতিবেদনে উল্লিখিত পক্ষ:' : 'Reported subject:'}
          </p>
          <p className="font-[var(--font-weight-semibold)] text-ui-content-primary truncate max-w-full">
            {report.reportedSubject}
          </p>
        </div>
      )}
      ...
      {shouldShowDescription && (
        <p className="text-[var(--type-fixed-13)] sm:text-[var(--type-fixed-14)] md:text-[var(--type-fixed-16)] leading-[var(--type-line-ratio-150)] sm:leading-[var(--type-line-ratio-155)] md:leading-[var(--type-line-26)] text-ui-content-secondary line-clamp-2 md:line-clamp-3 font-[var(--font-weight-regular)] break-words">
          {shortDesc}
        </p>
      )}
      ...
      <div className="flex items-center justify-between gap-2 pt-2 sm:pt-2.5 md:pt-3 border-t border-ui-stroke-subtle text-[var(--type-fixed-12)] sm:text-[var(--type-fixed-13)] md:text-[var(--type-fixed-14)] text-ui-content-muted">
        ...
        <div className="flex items-center gap-1 sm:gap-2 md:gap-3 shrink-0 text-[var(--type-fixed-125)] sm:text-[var(--type-fixed-135)] md:text-[var(--type-fixed-16)]">
          ...
          >
            {isCopied ? (
              <span className="inline-flex items-center gap-1" aria-live="polite">
                ...
                <span className="text-ui-success-text font-[var(--font-weight-semibold)] text-[var(--type-fixed-115)] sm:text-[var(--type-fixed-13)] md:text-[var(--type-fixed-14)]">
          ...
          <Link
            ...
            className="inline-flex items-center gap-1 sm:gap-1.5 font-[var(--font-weight-semibold)] text-ui-content-primary hover:underline transition-colors py-1.5 px-1 min-h-[44px] focus:outline-none focus-visible:ring-2 focus-visible:ring-ui-focus rounded-[var(--radius-badge-md)]"
          >
