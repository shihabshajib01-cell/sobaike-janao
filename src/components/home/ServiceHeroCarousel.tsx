...
export interface ServiceSlide {
  ...
  primaryCtaBn: string;
  primaryCtaEn: string;
  path: '/harassment' | '/rickshaw' | '/extortion';
  reportCount?: number;
}
...
export const ServiceHeroCarousel: React.FC<ServiceHeroCarouselProps> = ({
  ...
  const conf = SECTIONS[currentSlide.key];
  const count = reportCounts[currentSlide.key] ?? 0;
  ...
  return (
    <div id={id} className={`hidden md:block space-y-3.5 ${className}`}>
      {/* Tablet & Desktop Interactive Hero Banner (>= 768px) */}
      <section
        tabIndex={0}
        role="region"
        ...
        style={{
          borderLeftColor: conf.primaryColor,
          borderLeftWidth: '4px',
        }}
      >
        <div
        ...
        <div className="relative z-10 p-6 md:p-7 space-y-4">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            >
              <FeatureIcon section={currentSlide.key} size="md" />
            </div>
          ...
          <div className="space-y-1.5 text-left pt-1">
            <h2 className="text-[24px] md:text-[26px] font-extrabold leading-[1.3] text-primary tracking-tight">
          ...
          <div className="flex items-center justify-between gap-4 pt-2">
            ...
            <div
              ...
              className="flex items-center gap-1 bg-surface-subtle border border-subtle rounded-xl p-1 shrink-0 shadow-2xs"
            >
              <button
